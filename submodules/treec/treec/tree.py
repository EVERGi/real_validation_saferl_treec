...
import abc
from treec.utils import normalise_input
...
class BinaryTree:
    def __init__(
        self, node_array: List[float], feature_info: List[str], action_names: List[str]
    ):
    ...
    def get_action_with_unnormalised_input(self, feature_values: List[float]):
        norm_input = list()
        for j, feature in enumerate(feature_values):
            input_info = self.feature_info[j]
            low_bound = input_info[1][0]
            high_bound = input_info[1][1]
            norm_input.append(normalise_input(feature, low_bound, high_bound))

        node = self.get_action(norm_input)
    ...
    def save_tree_as_free(self, filepath):
        str_model = self.get_str_model()

        file = open(filepath, "w+")
        file.write(str_model)
        file.close()
    ...
    def get_str_model(self):
        ...
        str_model = f"{node_array}\n"
        str_model += f"{self.feature_info}\n"
        str_model += f"{self.action_names}\n"
    ...
    def get_dict_model(self):
        ...
        dict_model = dict()
        dict_model["node_array"] = node_array
        dict_model["feature_info"] = self.feature_info
        dict_model["action_names"] = self.action_names
...
def construc_tree_from_file(filepath):
    ...
    feature_info = ast.literal_eval(content[1][:-1])
    action_names = ast.literal_eval(content[2][:-1])

    return BinaryTreeFree(node_array, feature_info, action_names)
...
class BinaryTreeFree(BinaryTree):
    def __init__(
        ...
        super().__init__(node_array, feature_info, action_names)
...
class BinaryTreeFreeCont(BinaryTreeFree):
    def __init__(
        ...
        super().__init__(node_array, feature_info, [])
