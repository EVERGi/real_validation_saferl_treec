        ...
        for algo in algos_to_plot:
            ...
            for i in range(4):
                ...
                if i == 0:
                    ...
                    label_blue = None
                    label_orange = None
                plt.plot(x, comp_cost[start:end], label=label_blue, color=blue_color)
                plt.plot(
            ...
            plt.tight_layout()
            if algo == "MPC" and comp == "Simul" and save_fig:
...
def plot_comparison_results(
    log_folder, log_perfect="results/experiment_1_perfect/", plot_all_comparison=True
):
    ...
    if plot_all_comparison:
        plot_cost_comparison(all_costs)


def plot_original_comparison_result():
    log_folder = "results/experiment_1/"
    plot_comparison_results(log_folder, plot_all_comparison=True)
...
def plot_data_cons():
    ...
    log_perfect = "results/perfect_mpc_data_cons/"
    plot_comparison_results(
        log_folder, log_perfect=log_perfect, plot_all_comparison=False
    )


def plot_bug_mpc_and_nobug():
    log_folder = "results/experiment_1/"
    log_bug = "results/bug_mpc_new_start_day_8_house_1/"
    plot_comparison_results(log_folder, log_perfect=log_bug, plot_all_comparison=True)


def plot_gym_results():
    log_folder = "results/experiment_1_gym/"
    plot_comparison_results(log_folder, plot_all_comparison=True)


def plot_final_results():
    log_folder = "results/final_results/"
    plot_comparison_results(log_folder, plot_all_comparison=True)


def plot_house_3_tree_progress():
...
def print_real_energy_peak():
    ...
    peak = {ems: [] for ems in emss}
    grid_import = {ems: [] for ems in emss}
    grid_export = {ems: [] for ems in emss}
    ...
    for house_num in houses:
        ...
        for ems in emss:
            ...
            max_power = max(result_data["grid_import"])
            # print(f"{ems} house {house_num}: {max_power}")
            peak[ems].append(max_power)
            sum_grid_import = sum(result_data["grid_import"]) * 0.25  # In kWh
            sum_grid_export = sum(result_data["grid_export"]) * 0.25  # In kWh

            grid_import[ems].append(sum_grid_import)
            grid_export[ems].append(sum_grid_export)

    for ems in emss:
        print(f"{ems} peak: {np.average(peak[ems])}")
        print(f"{ems} sum import: {sum(grid_import[ems])}")
        print(f"{ems} sum export: {sum(grid_export[ems])}")
        import_minus_export = sum(grid_import[ems]) - sum(grid_export[ems])
        print(f"{ems} import - export: {import_minus_export}")


if __name__ == "__main__":
    plot_house_3_tree_progress()
    plot_final_results()
    plt.show()
